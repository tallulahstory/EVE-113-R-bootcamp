...
```{r Question_1}
getwd()
setwd("/Users/loganstory/Desktop/PhD/Teaching/R Bootcamp/R-bootcamp") # should be students wd

library(palmerpenguins)

# data should have a descrptive data frame object name later down the line, but saved as 'data' for now
...
```{r Question_3c}
names(tide_pool_data) <- c("pool_number",             # all columns should be using all lowercase
                           "volume", 
                           "tide_pool_size", 
...
```{r Question_6b}
arrange(tide_pool_data, desc(macro_shannon_mean), desc(macro_richness))
```
...
```{r Question_7a}
(tide_pool_subset <- select(tide_pool_data,          # bonus if each line (yay tidy code practices!!)
                           pool_number, 
                           macro_shannon_sessile, 
                           macro_shannon_mobile, 
                           macro_richness))
...
```{r Question_8b}
tide_pool_data <- rename(tide_pool_data,               # df can be renamed, or overwrite previous df
