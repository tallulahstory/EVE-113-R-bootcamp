...
!['iwa (Great Frigate) bird in flight in Mangroves \| Photo Credit: Lorenzo Mittiga](images/clipboard-4261253529.png)
...
4.  Be able to set evaluations and customization in .rmd code chunks with `knitr`.
...
1a. Type something, anything, into the Console then hit 'Enter' or 'return' on your keyboard. What happens?

1b. Why does this happen when typing something random?
:::

## Setting your working directory

Now, we're going to type out our first ever REAL command into the **Console**! Yay! Go ahead and either manually type or copy/paste the command below then press 'return' or 'Enter' on your keyboard.
...
For example, if I wanted to change my working directory to an existing folder called `Coding Tutorials` located on my `Desktop` in a folder called `EVE 113`, I’d run the following code:

```{r, eval = FALSE}
setwd("/Users/yourusernamehere/Desktop/EVE 113/Coding tutorials")
```

This is the directory path R will now use when searching and saving files as I work: **Users** \> **yourusernamehere** \> **Desktop** \> **EVE 113** \> **Coding Tutorials**. The "\>" indicates the directionality that R is searching in my computer for the works pace.
...
For the sake of this course (and these tutorials), I'd like you to make a folder to set as your working directory before we proceed any further. Use the following template:
...
```{r, eval=FALSE}
setwd("/Users/yourusername/Desktop/EVE 113/Coding Tutorials") # on macOS
```

or

```{r, eval=FALSE}
setwd("C:\Users\yourusername\Desktop\EVE 113\Coding Tutorials") # on Windows
```

- Here the working directory is explicitly specified (`/Users/yourusername/Desktop/EVE 113`).

- It includes the set folder destination for a file (`/Coding Tutorials`, or essentially where you want to go with the path).

A **relative** path would be something like...

```{r, eval=FALSE}
setwd("./Coding Tutorials")
```

- Here, the working directory is implicitly specified.

- A `.` before the forward or backslash indicates you are moving *down* the path

- A `..` indicates movement *up* the path.

Return to your parent directory by typing the following:

```{r, eval = FALSE}
setwd("..") 
```

Move laterally to another folder in your working directory:

```{r, eval = FALSE}
setwd("../neightbor folder")
```
...
1.  Set your working directory.
    ...
    ```{r, eval = FALSE}
    setwd("/Users/yourusenamehere/Desktop/EVE 113/Coding Tutorials")
    ```
...
::: callout-note
**For your R Preparedness assignment, you will be completing your work in a pre-prepared .rmd file. This output option will already be chosen for you in those files.**

For scratch work on these tutorials, set the knit output to whatever you'd like it to be
:::
...
Find the Visual editor viewer. Switch between visual and Source a few times and notice the differences. What do the two \## prior to text in the Source editor mean? What does this do to the text in the Visual editor?
:::
...
So, whenever we generate a plot or need a summary from our code chunk output, navigating the printed results in the code editor will become cumbersome.

::: callout-important
**For the purposes of continuity in this class, we will be setting our [code chunk outputs]{.underline} to be in the *Console* instead of *inline*.**
:::
...
3a. In the same code chunk as the summary(cars) code, retrieve a summary of the data set pressure (the data set you plotted above).

3b. Add a comment about what running the code in 3a showed you.

3c. In the same code chunk as the plot(pressure) code, plot the cars data set.
:::

## RMarkdown Chunk Options

At the end of the document around line 32 there is the following text explaining the code chunk options.
...
In the plot code chunk, edit the chunk option so that when kitting, none of the source code appears.
:::
...
> (e.g., `/Users/yourusername/Desktop/EVE 113/Coding Tutorials`).

Now, when you turn in the R-Prep.rmd assignment, you will need to knit your code and save it as and html.
...
1a. R should provide an error, something along the lines of:

> Error: object 'whatever you typed here' not found

1b. This happens because I'm assuming whatever you typed isn't an actual command or saved object in R. You should think of R as a hungry, hungry hippo: The hippo will only work for you if you feed it the right stuff. If you don't feed it the right stuff then it won't work (or might even eat you).
:::

::: {.callout-important appearance="simple" icon="false"}
**DIY T0-2**

You know you've successfully found the right thing when you look beneath the document tab and before your actual script (sandwiched in the left corner) and click onto what looks more like a real document and not code.

The two \##'s prior to the text in the source editor turns plain text larger and bolder. In the visual editor, this displays the text as "Header 2".
:::
...
```{r, eval = FALSE}
summary(cars)
summary(pressure) # shows the temperature and pressure of cars
```
...
Your chunk option should look like the below, and when kitting, your final html should not show the source code producing both the pressure and cars plots.
