...
Returning to our running example: do Adelie and Gentoo penguins differ in flipper length? This is a statistical test that relies on data being normally distirbuted and homoscedastic. We can do this in two seperate pipe strings using rstatix...
...
Flipper length is not normally distributed for the two species, but the assumption of equality of variances is satisfied by a Levene's Test. If our variances are not equivalent, we can instead choose a Welch's t-test, which does not assume equality of variances or normality of distributions. Here, we will be using the t_test() command, which can be altered to run a Welch's t-test or a Students t-test...
...
There's a lot to take in when you generate a summary table from an ANOVA, but the primary thing to look for is the Pr(\>F) value for your predictor variable. The Pr(\>F) is the p-value associated with your F-statistic in the ANOVA table, specifically answering the question "if there really were no difference in group means, what's the probability of getting an F-statistic this larger, or maybe even larger, by chance?" A low p-value indicates it's less by chance, and that it's more representative of the data. The F-value represents the variance *between* group means. A large F is indicative that group means are spread out relative to how much individual vary within that group.
...
The table output tells us that Adelie and Gentoo and Chinstrap and Gentoo are significantly different, but Adelie and Chinstrap do not differ in mean body mass. We can add this onto a boxplot with significance bars using ...
